...
public class Player {
     ...
     * isDoLock,锁定WhatDo
     */
    private boolean isAlive, isDoLock = false;
    ...
    public Player(int x, int y, int width, int height, int play_equip_C, int HP_now, int MP_now) {
        ...
        this.width = width;
        this.height = height;
        //ground表示草地边界矩形宽800,高45
        ground = new Rect(0, 600, 0 + 800, 600 + 251);
    ...
    public void update(float delta) {
        ...
        if (totalDuration > 0) {
            ...
            if (temp == 0 && DoWhat != 0) {
                ...
                switch (WhatDo % 10) {
                    case 2:
                        PlayState.y = PlayState.y + playSpeed_Y * delta * temp;//1+3
                    case 1:
                        PlayState.x = PlayState.x + playSpeed_X * delta * temp;
                        break;
                    case 4:
                        PlayState.x = PlayState.x - playSpeed_X * delta * temp;//3+5
                    case 3:
                        PlayState.y = PlayState.y + playSpeed_Y * delta * temp;
                        break;
                    case 6:
                        PlayState.y = PlayState.y - playSpeed_Y * delta * temp;//5+7
                    case 5:
                        PlayState.x = PlayState.x - playSpeed_X * delta * temp;
                        break;
                    case 8:
                        PlayState.x = PlayState.x + playSpeed_X * delta * temp;//1+7
                    case 7:
                        PlayState.y = PlayState.y - playSpeed_Y * delta * temp;
                        break;
                    //计算屏幕原点的像素位置
            ...
            if (temp == 3) {
                ...
                switch (WhatDo % 10) {
                    case 2:
                        AttackTempY = coordinate[1] - 1;
                    case 1:
                        AttackTempX = coordinate[0] - 1;
                        break;
                    case 4:
                        AttackTempX = coordinate[0] + 1;
                    case 3:
